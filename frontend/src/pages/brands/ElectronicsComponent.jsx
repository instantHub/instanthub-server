...
function ElectronicsComponent({ data }) {
  return (
    <div>
      {/* {data.map((category, index) => (
        <div key={index}>
          {Object.keys(category).map((categoryName, categoryIndex) => (
            <div key={categoryIndex}>
              <h2>{categoryName}</h2>
              {Object.keys(category[categoryName]).map(
                (subcategoryName, subcategoryIndex) => (
                  <div key={subcategoryIndex}>
                    <h3>{subcategoryName}</h3>
                    {category[categoryName][subcategoryName].map(
                      (item, itemIndex) => (
                        <div key={itemIndex}>
                          {Object.keys(item).map(
                            (sectionName, sectionIndex) => (
                              <div key={sectionIndex}>
                                <h4>{sectionName}</h4>
                                <ul>
                                  {Array.isArray(item[sectionName])
                                    ? item[sectionName].map(
                                        (detail, detailIndex) => (
                                          <li key={detailIndex}>{detail}</li>
                                        )
                                      )
                                    : Object.keys(item[sectionName]).map(
                                        (detailName, detailIndex) => (
                                          <div key={detailIndex}>
                                            <h5>{detailName}</h5>
                                            <ul>
                                              {Array.isArray(
                                                item[sectionName][detailName]
                                              )
                                                ? item[sectionName][
                                                    detailName
                                                  ].map(
...
export default function App() {
  const data = [
    // Insert provided JSON data here
      ...
      //               "Battery in Service (Health is less than 80%)",
      //               "Speaker Faulty",
      ...
      //           },
      //           {
      //             "Please select the accessories available!": [
      ...
      //         brand: "Microsoft",
      //         model: "Surface Pro 7",
      //         variants: ["i5 8GB RAM", "i7 16GB RAM"],
      //         condition: "Used",
      //         quoted_amount: 899,
      //       },
      //     ],
  ...
  return <ElectronicsComponent data={data} />;
}
